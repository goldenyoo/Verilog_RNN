...
for data_label = data_labels
    ...
    for j = length(Class_2):-1:1
        Class_2 = [Class_2 Class_2(j)-5 Class_1(j)-3 Class_1(j)+3 Class_2(j)+5];
    end
    
    for j = length(Class_3):-1:1
        Class_3 = [Class_3 Class_3(j)-5 Class_1(j)-3 Class_1(j)+3 Class_3(j)+5];
    end
    
    for j = length(Class_4):-1:1
        Class_4 = [Class_4 Class_4(j)-5 Class_1(j)-3 Class_1(j)+3 Class_4(j)+5];
    end
    %%
    m = 30;
    for i = 1:length(Class_1)
        X1(:,:,i+c1) = my_hjorth(s(Class_1(i):Class_1(i)+313,[2 6 7 8 9 11 12 13 14 18]),m);
        Y1(i+c1,1) = 1;
    end
    
    for i = 1:length(Class_2)
        X2(:,:,i+c2) = my_hjorth(s(Class_2(i):Class_2(i)+313,[2 6 7 8 9 11 12 13 14 18]),m);
        Y2(i+c2,1) = 2;
    end
    
    for i = 1:length(Class_3)
        X3(:,:,i+c3) = my_hjorth(s(Class_3(i):Class_3(i)+313,[2 6 7 8 9 11 12 13 14 18]),m);
        Y3(i+c3,1) = 3;
    end
    
    for i = 1:length(Class_4)
        X4(:,:,i+c4) = my_hjorth(s(Class_4(i):Class_4(i)+313,[2 6 7 8 9 11 12 13 14 18]),m);
        Y4(i+c4,1) = 4;
    end
